...
public class MyCommentFragment extends BaseListFragment<UserActivity> {
    ...
    public void initView(View view) {
        ...
        mAdapter.setFinishText(R.string.no_more_comment);

        title.setTitleTxt(getString(R.string.my_comment));
        ...
        mErrorLayout.setEmptyImage(R.drawable.no_data_comment);
        if (ClientInfo.isLoginUser(mUid)) {
            mErrorLayout.setNoDataContent(getString(R.string.no_comment));
    ...
    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart(UmengConstants.MY_COMMENT_FRAGMENT); //统计页面
    }

    @Override
    public void onPause() {
        super.onPause();
        MobclickAgent.onPageEnd(UmengConstants.MY_COMMENT_FRAGMENT);
    }

    @Override
    protected MyCommentAdapter getListAdapter() {
        return new MyCommentAdapter(getActivity(), mUid);
    }
    ...
    protected void requestData(boolean refresh) {
        if (refresh) {
            MineApi.getMyActivity(mUid, 1, Messages.ACTION_COMMENT_COSPLAY, null, mHandler);
        } else {
            sendRequestData();
        }
    }
    ...
    protected void sendRequestData() {
        ...
        MineApi.getMyActivity(mUid, 1, Messages.ACTION_COMMENT_COSPLAY, lastid, mHandler);
    }
