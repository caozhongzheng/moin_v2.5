...
/**
 * 话题详情最热列表适配器[3.2.7版本以前]
 * Created by moin on 16/3/25.
 */
public class TopicDetailHotAdaptor extends ATopicDetailAdapter {
    private static final ILogger MyLog = LoggerFactory.getLogger(TopicDetailHotAdaptor.class.getSimpleName());
...
//    private static int RL_IMG_TOPMARGIN = - IMG_WIDTH * 13 / 100;
//    private static int LL_CONTENTINFO_TOPMARGIN = IMG_WIDTH * 57 / 100;
    ...
    protected View getRealView(int position, View convertView,
        ...
        params.topMargin = 0;//RL_IMG_TOPMARGIN;

        params = (ViewGroup.MarginLayoutParams) vh.mLyLike.getLayoutParams();
        params.topMargin = (int) (IMG_WIDTH - TDevice.dpToPixel(49));//LL_CONTENTINFO_TOPMARGIN;

        params = (ViewGroup.MarginLayoutParams) vh.mLlContentInfo.getLayoutParams();
        params.topMargin = IMG_WIDTH;//LL_CONTENTINFO_TOPMARGIN;

        final CosplayInfo item = mDatas.get(position);
        MyLog.i("getRealView start ["+position+"]" + (item == null ? null : item.toString()));
    ...
    private void updateCosplayInfo(ViewHolder vh, CosplayInfo cosplay, Context context, int position, boolean animShow) {
        ...
        // 发布的图,大咖秀
        try {
            ViewGroup.LayoutParams paramsImage = vh.mIvCosplayImage.getLayoutParams();
            ViewGroup.LayoutParams paramsContainer = vh.mRlImage.getLayoutParams();
            paramsImage.width = paramsImage.height =
                    paramsContainer.width = paramsContainer.height
                            = IMG_WIDTH;
...
//        });
    ...
    private void updateBottomInfo(ViewHolder vh, CosplayInfo cosplay, Context context) {
        ...
        View.OnClickListener viewClick = null;
        ...
        View.OnClickListener forwardClick = v -> {
            MyLog.i("跳到改图转发界面(根据权限)=" + cosplay.getWriteAuth());
            if (cosplay.getWriteAuth() == 4) {
                cosplay.setIsWrite(1);
            }
            UIHelper.editCosplay(context, cosplay, TimeUtils.getCurrentTimeInLong());
        };
