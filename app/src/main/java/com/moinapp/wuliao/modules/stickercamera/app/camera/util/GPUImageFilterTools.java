...
//    public enum FilterType {
//        NORMAL,
//        GRAYSCALE, SATURATION, SEPIA,  BOX_BLUR, LOOKUP_AMATORKA}
    ...
    // 对比度的默认值1 范围(0-4),所以正常时默认刻度(0-100)为25,但是ui要求默认在中间(50)
    // 所以需要这么处理,progress=50 返回1.0, 如果小于50和大于50,分别计算
    public static float convertContrast(int progress) {
        ...
        if (progress < 0 || progress > 100 || progress == 50) {
            ...
            if (progress < 50) {
                contrast = progress * 0.02f;
            } else {
                contrast = 1.0f + (progress - 50) * 0.06f;
            }
        }
        return contrast;
    }

    public static  float convertBrightness(int progress) {
        // -1.0 to 1.0, with 0.0 as the normal level
        float brightness = 0;

        if (progress >= 0 || progress <= 100) {
            brightness = progress * 0.02f - 1;
        }
        return brightness;
    }

    public static float convertSaturation(int progress) {
    ...
    // 对比度的默认值1 范围(0-4),所以正常时默认刻度(0-100)为25,但是ui要求默认在中间(50)
    // 所以需要这么处理,如果contrast=1 刻度为50, 如果小于1和大于1,分别计算
    public static int convertContrastProgress(float contrast) {
        //0.0 to 4.0, with 1.0 as the normal level
        int progress = 0;

        if (contrast < 0 || progress > 4.0 || Math.abs(contrast - 0.0) < 0.001) {
            progress = 50;
        } else {
            if (contrast < 1) {
                progress = (int)(Math.ceil(contrast * 50));
            } else {
                progress = 50 + (int)(Math.ceil((contrast -1)/0.06f));
            }
        }
    ...
    public static  int convertBrightnessProgress(float brightness) {
        // -1.0 to 1.0, with 0.0 as the normal level
        int progress = 0;

        if (brightness >= -1.0 || progress <= 1.0) {
            progress = (int)(Math.ceil((brightness+1) * 50));
        }
        return progress;
    }

    public static int convertSaturationProgress(float saturation) {
        //0.0 - 2.0, with 1.0 as the default
        int progress = 0;

        if (saturation >= 0.0 || progress <= 2.0) {
            progress = (int)(Math.ceil(saturation * 50));
        }
        return progress;
    }
}
