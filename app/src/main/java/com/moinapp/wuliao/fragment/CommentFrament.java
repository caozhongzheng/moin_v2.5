...
public class CommentFrament extends BaseListFragment<Comment> implements
    ...
    public void onCreate(android.os.Bundle savedInstanceState) {
        ...
        if (args != null) {
            ...
            mId = args.getInt(Constants.BUNDLE_KEY_ID, 0);
            mOwnerId = args.getInt(Constants.BUNDLE_KEY_OWNER_ID, 0);
            mIsBlogComment = args.getBoolean(Constants.BUNDLE_KEY_BLOG, false);
        }
    ...
    public void onResume() {
        super.onResume();
        MobclickAgent.onPageStart(UmengConstants.COMMENT_FRAGMENT); //统计页面，
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            Comment comment = data
                    .getParcelableExtra(Constants.BUNDLE_KEY_COMMENT);
            if (comment != null) {
    ...
    @Override
    public void onClickSendButton(String text) {
