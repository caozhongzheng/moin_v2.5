...
//    private static String API_URL = "http://www.oschina.net/%s";
     public final static String HOST = "dev.mo-image.com";
     private static String API_URL = "https://dev.mo-image.com/v2/%s";
    ...
    private static void addUserHead() {
        Context context = BaseApplication.context();
        String uid = ClientInfo.getUID();
        String passport = ClientInfo.getPassport();
        String imei = MobileInfo.getImei(context);
        String aid = ClientInfo.getPushChannel();
        String channel = ClientInfo.getChannelId();
        client.addHeader(LoginConstants.IMEI, imei);
        if (!TextUtils.isEmpty(uid)) {
            client.addHeader(LoginConstants.UID, uid);
        } else {
            client.addHeader(LoginConstants.AID, aid);
            client.addHeader(LoginConstants.UID, null);
        }
        if (!TextUtils.isEmpty(passport)) {
            client.addHeader(LoginConstants.MOIN_PASSPORT, passport);
        }
        client.addHeader(LoginConstants.CID, channel);
//        client.addHeader("devicewidth", String.valueOf(MobileInfo.getDisplayWidth(context)));
        Log.i("user head", "add header:imei=" + imei + ",uid=" + uid + ", moin passport=" + passport + ", " +
